...
class WebsiteScraper(BaseScraper):
    ...
    def _parse_contacts(self, soup):
        ...
        main_phone = soup.find(lambda tag: tag.name == 'p' and main_phone_text in tag.get_text(strip=True).replace('\xa0', ' '))
        if main_phone:
        ...
        student_block = soup.find(lambda tag: tag.name == 'p' and 'СТУДЕНТАМ ЧГМА' in tag.get_text(strip=True).replace('\xa0', ' '))
        if student_block:
    ...
    def _parse_main_schedule(self, soup):
        ...
        sections = [
            ...
            ("Сдача анализов (Диагностическая поликлиника)", tables[1] if len(tables) > 1 else None),
            ("Сдача анализов (Бактериологическая лаборатория, ул. Бабушкина, 46)", tables[2] if len(tables) > 2 else None)
        ]
